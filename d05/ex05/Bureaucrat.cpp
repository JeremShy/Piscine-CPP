...
void					Bureaucrat::signForm(Form & f) const {
	std::cout << *this << " signs form " << f << std::endl;
	try {
		f.BeSigned(*this);
	} catch (Form::GradeTooLowException & e) {
		std::cout << *this << " failed to sign form " << f << " because his grade is too low." << std::endl;
...
void					Bureaucrat::executeForm(Form const & f) const {
	try {
		...
		std::cout << *this << " couldn't execute form " << f << " because the form isn't signed" << std::endl;
		throw (OfficeBlock::ExecutingException());
...
std::ostream& operator<<(std::ostream& os, const Bureaucrat & rhs) {
	os << "Bureaucrat " << rhs.getName() << " (Grade " << rhs.getGrade() << ")";
	return (os);
}
